...
%\newcommand\lirong[1]{\footnote{\bf Lirong: #1}}
\newcommand\lirong[1]{}
...
%\author{Tracking Number: 236} %\institute{Anonymous University, email: anon}}
...
1198 lb.
%As a second example, in the recent Netflix competition,
%the winning entry from the
%BellKor’s Pragmatic Chaos team combined together
%the ``wisdom'' of the previously rival
%BellKor, Pragmatic Theory and Chaos teams
%to predict user ratings for films.
%
%
...
%Whilst combining voting rules
%together cannot escape the
%reach of impossibility results like the
%Gibbard-Satterthwaite theorem, it can make it
%computationally harder for the manipulators
%to find any manipulation.
On the experimental
...
%that construct close to optimal manipulations.
...
%The winner
%is the candidate whose worst performance in pairwise
%elections is best.
...
%\item[Nanson's and Baldwin's rules:] These are iterated versions
%of the Borda rule. In Nanson's rule, we compute the Borda scores
%and eliminate any candidate with less than half the mean score.
%We repeat until there is an unique winner. In Baldwin's rule,
%we compute the Borda scores
%and eliminate the candidate with the lowest score.
%We again repeat until there is an unique winner.
...
%\item[Anonymity:] Voters are treated
%the same. More precisely, if we change the order of the voters then the
%winner does not change.
%\item[Neutrality:] Candidates are treated the
%same. More precisely, if we permute
%the names of two candidates then
%the winner is just the permutation
...
%This list of normative properties is not exhaustive.
% (though it
%represents some of the more important properties).
...
%For example,
%only one rule needs
%to be Condorcet consistent for the combination
%to be also.

%\begin{proposition}[Neutrality]
%If all of $X_1$ to $X_k$ and the tie-breaking
%mechanism are neutral then $\winners{X_1}{\ldots}{X_k}$ is
%also.
%\end{proposition}

%\begin{proposition}[Anonymity]
%If all of $X_1$ to $X_k$ and the tie-breaking
%mechanism are anonymous then $\winners{X_1}{\ldots}{X_k}$
%is also.
%\end{proposition}
...
%If all of $X_1$ to $X_k$ respect
%the property
%then $\winners{X_1}{\ldots}{X_k}$ does also.
...
%combining together voting rules.
%In particular, both monotonicity
%and consistency can be lost by the
%introduction of a runoff.
...
monotonic.
% as increasing a preference
%for the winner made them lose.
\myqed

We give a stronger %negative
result for
...
by Bartholdi, Tovey and Trick %in an influential paper
...
% where manipulation is NP-hard, but for their combination manipulation is polynomial to compute; we also examples, where manipulation of the base rules is
%NP-hard but manipulation of their combination
%is polynomial, as well as
%% some artificial voting rules
%where manipulation of the base rules is
%polynomial but manipulation of their combination
%is NP-hard.
...
Computing a manipulation of $X$ %that elects $-1$
is NP-hard.
...
%Suppose there is a set of $k$
%manipulating votes $M$ such that $c$ wins $P \cup M$ under $\winner{plurality}{veto}$.
%In this case $c$ wins under $plurality$ or under $veto$. Suppose $c$ wins under $plurality$ and some other candidate $a$ wins
%under $veto$.
...
%Suppose $c$ wins $P\cup M$ under $veto$ and  candidate $a$ wins
%under $plurality$.
...
a vertex in the first partition. % labelled by the candidate
For each position in $[1,m] \setminus \{i,j\}$
we introduce a vertex in the second partition. % labelled by their positions.
...
$k$-approval, manipulation %with a single manipulator
remains NP-hard
to compute.
...
scores of $c$ and $g'$ are the same and $c$ wins %the run-off
by tie-breaking.
\myqed
...
votes.
% We can not increase score of $c$ as
% we already placed $c$ in the first position.
% We also know that there is a winner in this
% round. If $c$ is not a winner then regardless
% of the manipulator votes $a$ or $b$ wins.
%
\myqed
...
$N_{P \cup M}(a,c) > N_{P \cup M}(c,a)$ and
$N_{P \cup M}(b,c) \leq N_{P \cup M}(c,b)$.
%In this case, for $c$ to win  manipulators
%need to make sure that $a$ does not reach the final
...
manipulation is intractable to compute %with 3 candidates,
...
mechanism to increase the %computational
complexity
of manipulation.
...
minimise the number of manipulators. % required.
%\cite{Zuckerman09:Algorithms}.
...
number of manipulators \cite{zuckermanSODA08}. %\cite{xzpcrijcai09}.
...
Eggenberger urn model. %~\cite{bergPC85}.
In the urn model,
...
loose. %, since any candidate can be the winner of either $X$ or $Y$
%and meet the preferred candidate in the runoff round.
...
that %our adaptation of $\bordashort$
...
%\bibliography{/Users/twalsh/Documents/biblio/a-z,/Users/twalsh/Documents/biblio/a-z2,/Users/twalsh/Documents/biblio/pub,/Users/twalsh/Documents/biblio/pub2}
%\bibliography{a-z2,pub2}
